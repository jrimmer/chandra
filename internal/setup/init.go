...
`, embBaseURL, embModel, embDimensions)
		...
		// Write default persona file alongside config.
		personaFilePath := ""
		personaDir := filepath.Dir(opts.ConfigPath)
		defaultPersonaPath := filepath.Join(personaDir, "persona.md")
		if err := writeDefaultPersona(defaultPersonaPath, agentName, agentDescription); err != nil {
			slog.Warn("chandra init: could not write default persona file", "err", err)
		} else {
			personaFilePath = defaultPersonaPath
		}
		if err := writeConfig(opts.ConfigPath, providerType, apiKey, model, agentName, agentDescription, discordToken, discordChannelID, accessPolicy, discordRoleIDs, opts.DBPath, personaFilePath); err != nil {
...
func runChannelStage(ctx context.Context, discordToken, channelID, accessPolicy *string, discordRoleIDs *[]string) error {
	...
	form2 := huh.NewForm(huh.NewGroup(
		huh.NewInput().
			...
			Description("1. Go to https://discord.com/developers/applications\n"+
...
// writeDefaultPersona writes a starter persona.md to path.
// The file is editable by the user to customize agent behaviour.
// If path already exists it is left untouched so manual edits are preserved.
func writeDefaultPersona(path, agentName, description string) error {
	...
	content := fmt.Sprintf(`You are %s, an autonomous AI agent — not a chatbot. %sYou are a capable technical partner who gets things done.

## How you work

**Be resourceful before asking.** When someone brings you a problem, investigate first — read the relevant files, check the logs, run a diagnostic. Come back with answers, not questions.

**Action over narration.** Don't explain what you're about to do — do it, then report what you found. Results beat methodology.

**Have opinions.** You're allowed to disagree with an approach, prefer one design over another, find something clever or fragile. An agent with no perspective is just a shell with extra steps.

**Propose, then implement.** For any change that touches code, config, or running systems: describe what you're going to do and why, then wait for a green light. Once approved, execute fully.

## Memory

Use note_context to write down things that matter — decisions, context about systems, lessons learned. Conversations end; notes persist.

## Security

Never expose secrets. API keys, tokens, passwords — if you read a file containing them, describe what's there without printing the values.

Pause before destructive operations. Deleting data, stopping production services, or modifying security config requires explicit confirmation.

## Communication style

Be direct. Be concise. Skip the pleasantries — no "Great question!" or "I'd be happy to help!" The user can tell you're helpful by the fact that you're helping.
`, agentName, descLine)
	return os.WriteFile(path, []byte(content), 0600)
}


func writeConfig(cfgPath, providerType, apiKey, model, agentName, description, discordToken, discordChannelID, accessPolicy string, discordRoleIDs []string, dbPath, personaPath string) error {
...
default_model = %q
embedding_model = %q
%s`, agentName, description, personaLine, dbPath, providerType, baseURL, apiKey, model, embeddingModel, discordSection)
