...
func mustClient() *api.Client {
	return api.NewClient(api.SocketPath())
}
...
// skillSummary is a local mirror of skills.Skill for JSON decoding in the CLI.
// Kept separate so the client binary doesn't import the full skills package.
...
// printSkillSummary prints a one-line summary (for skill list).
func printSkillSummary(s skillSummary, indent string) {
	cat := s.Category
	if cat == "" {
		cat = "—"
	}
	cronMark := ""
	if s.Cron != nil {
		cronMark = "  ⏱ " + s.Cron.Interval
	}
	genMark := ""
	if s.Generated != nil {
		genMark = "  [generated]"
	}
	fmt.Printf("%s%-22s  %-14s  %s%s%s\n", indent, s.Name, cat, s.Description, cronMark, genMark)
}

// printSkillDetail prints a full review card (for skill show / skill pending).
func printSkillDetail(s skillSummary) {
	genLine := ""
	if s.Generated != nil {
		by := s.Generated.By
		if by == "" {
		...
		genLine = fmt.Sprintf("  [generated by %s", by)
		if s.Generated.CreatedAt != "" {
			genLine += " · " + s.Generated.CreatedAt
		}
		genLine += "]"
	}
	status := ""
	if s.Generated != nil {
		status = "  status: " + s.Generated.Status
	}
	...
	for _, b := range s.Requires.Bins {
		requires = append(requires, "bin:"+b)
	}
	for _, t := range s.Requires.Tools {
		requires = append(requires, "tool:"+t)
	}
	for _, e := range s.Requires.Env {
		requires = append(requires, "env:"+e)
	}
	if len(requires) > 0 {
...
var skillListCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := client.Call(context.Background(), "skill.list", nil, &result); err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			os.Exit(1)
		}
		if len(result.Skills) == 0 && len(result.Unmet) == 0 {
			fmt.Println("No skills loaded.")
			return
		}
		if len(result.Skills) > 0 {
			fmt.Printf("%d skill(s) loaded:\n\n", len(result.Skills))
			for _, s := range result.Skills {
				printSkillSummary(s, "  ")
			}
		}
		if len(result.Unmet) > 0 {
			fmt.Printf("\n%d skill(s) with unmet requirements:\n", len(result.Unmet))
			for _, u := range result.Unmet {
				fmt.Printf("  %-20s  missing: %s\n", u.Name, strings.Join(u.Missing, ", "))
			}
		}
	},
}
...
var skillShowCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		client := mustClient()
		var s skillSummary
		if err := client.Call(context.Background(), "skill.show", map[string]string{"name": args[0]}, &s); err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			os.Exit(1)
		}
		printSkillDetail(s)
...
var skillPendingCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		client := mustClient()
		var result struct {
			Pending []skillSummary `json:"pending"`
		}
		if err := client.Call(context.Background(), "skill.pending", nil, &result); err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			os.Exit(1)
		}
		if len(result.Pending) == 0 {
			fmt.Println("No skills pending review.")
			return
		}
		fmt.Printf("%d skill(s) pending review:\n", len(result.Pending))
		for _, s := range result.Pending {
			fmt.Println()
			printSkillDetail(s)
			fmt.Println()
		}
		fmt.Println("─────────────────────────────────────────")
		fmt.Println("  chandra skill show <name>     full body")
		fmt.Println("  chandra skill approve <name>")
		fmt.Println("  chandra skill reject <name>")
	},
}
